...
if TYPE_CHECKING:
    from .foo.foo_request_builder import FooRequestBuilder
    from .item.with_path_request_builder import WithPathRequestBuilder
...
class ApiSdk(BaseRequestBuilder):
    ...
    def with_path(self,) -> WithPathRequestBuilder:
        """
        Builds and executes requests for operations under /{path}/
        Returns: WithPathRequestBuilder
        """
        from .item.with_path_request_builder import WithPathRequestBuilder

        return WithPathRequestBuilder(self.request_adapter, self.path_parameters)
    
    @property
    def foo(self) -> FooRequestBuilder:
        """
        The foo property
        """
        from .foo.foo_request_builder import FooRequestBuilder

        return FooRequestBuilder(self.request_adapter, self.path_parameters)
